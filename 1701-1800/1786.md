...
public:
    int countRestrictedPaths(int n, vector<vector<int>>& edges) {
        ...
        pq.push({n, 0});
        while (!pq.empty()) {
            auto [nd, _] = pq.top(); pq.pop();
            if (visited[nd]) continue;
            visited[nd] = true;
            for (auto itr : hash[nd]) {
                dists[itr.first] = min(dists[itr.first], dists[nd] + itr.second);
                pq.push({itr.first, dists[itr.first]});
            }
        }
